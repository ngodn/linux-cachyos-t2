...
### T2 MAC SUPPORT
# Enable T2 Mac support (Apple T2 chip hardware)
: "${_enable_t2:=yes}"
...
if _is_lto_kernel && [ "$_use_lto_suffix" = "yes"  ]; then
    _pkgsuffix=cachyos-lto-t2
elif ! _is_lto_kernel && [ "$_use_gcc_suffix" = "yes" ]; then
    _pkgsuffix=cachyos-gcc-t2
else
    _pkgsuffix=cachyos-t2
fi
...
# T2 Mac patches
if [ "$_enable_t2" = "yes" ]; then
    source+=("patches::git+https://github.com/t2linux/linux-t2-patches#commit=${T2_PATCH_HASH}")
fi
...
prepare() {
    ...
    # Apply T2 patches first if enabled
    if [ "$_enable_t2" = "yes" ]; then
    ...
    for src in "${source[@]}"; do
        ...
        if ! patch -Np1 --dry-run < "../$src" >/dev/null 2>&1; then
            ...
            patch -Np1 --forward --reject-file=- < "../$src" || echo "Partial application of $src"
        else
            patch -Np1 < "../$src"
        fi
    done
    ...
    # Apply T2-specific configuration if enabled
    if [ "$_enable_t2" = "yes" ]; then
        echo "Applying T2-specific kernel configuration..."
        # Get T2 patches extra config and apply
        if [ -f "$srcdir/patches/extra_config" ]; then
...
_package() {
    ...
    if _is_lto_kernel; then
        provides+=(linux-cachyos-lto-t2=$_kernver)
        replaces=(linux-cachyos-lto-t2)
    fi
...
_package-headers() {
    ...
    if _is_lto_kernel; then
        provides+=(linux-cachyos-lto-t2-headers=$_kernver)
        replaces=(linux-cachyos-lto-t2-headers)
